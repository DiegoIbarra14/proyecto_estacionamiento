...
function UpdateProducto(props) {
  ...
  const showProducto = () => {
    http.get(`/productos/get/${params.idProducto}`)
      .then((response) => {
        setProducto(response.data);
        // console.log(producto,"ES DE /productos/get/");
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const handleAddIngrediente = (materia_prima) => {
    if (materiaPrima === null) {
      ...
      if (!existe) {
        console.log("ingresa al IF para crear el estado producto");
        setProducto({
        ...
        console.log("ingresa al else para crear el estado producto");
        setProducto({
          nombre: producto.nombre,
          cantidad_base: producto.cantidad_base,
          presentacion_id: producto.presentacion_id,
  ...
  validarMaquinasConServicios()
    .then(() => {
      ...
      if (producto.nombre.length === 0) {
        showToast("error", "Error al editar Producto", `Debe ingresar un nombre al producto`);
        return;
      }

      if (producto.cantidad_base <= 0) {
        showToast("error", "Error al editar Producto", `Debe ingresar una cantidad base mayor a 0`);
        return;
      }

      if (producto.presentacion_id === null) {
        showToast("error", "Error al editar Producto", `Debe seleccionar una presentación para el producto`);
        return;
      }

      if (producto.tiempo_vida <= 0) {
        showToast("error", "Error al editar Producto", `Debe ingresar una cantidad de vida útil mayor a 0`);
        return;
      }

      if (producto.ingredientes.length === 0) {
        showToast("error", "Error al editar Producto", `Debe ingresar al menos un ingrediente para el producto`);
        return;
      }

      if (producto.maquinas.length === 0) {
        showToast("error", "Error al editar Producto", `Debe ingresar al menos una máquina para el producto`);
        return;
      }

      http.put(`productos/update/${params.idProducto}`, producto)
        .then((response) => {
          navigate("/getProductos", { state: { message: `Se editó el producto ${producto.nombre} correctamente`, severity: 'success', title: 'Producto Editado' } });
        })
        .catch((error) => {
          console.log(error);
          showToast("error", "Producto No Editado", `No se editó el producto ${producto.nombre}`);
        });
    })
    .catch((errores) => {
      if (Array.isArray(errores) && errores.length > 0) {
        const maquinasSinServicioText = errores.join(', ');
        showToast("error", "Error al editar Producto", `Las siguientes máquinas no tienen servicios asociados: ${maquinasSinServicioText}`);
      } else {
        console.error("Error inesperado en la validación de máquinas:", errores);
        showToast("error", "Error al editar Producto", "Error inesperado al validar máquinas");
      }
    })
    .finally(() => {
      setTimeout(() => {
